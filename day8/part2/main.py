...
def get_value(index):
  ...
  if data[index] > 0:
    # node has children
    children = get_immediate_children(index)
    
    for child in get_metadata(index):
      try:
        value += get_value(children[child-1])
...
print(get_value(0)) #print sum of metadata for root node
